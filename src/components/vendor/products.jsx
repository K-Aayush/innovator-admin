...
export function ProductsPage() {
  ...
  useEffect(() => {
    setPage(0);
  }, [search, selectedCategory, sortStock]);
  ...
  const fetchProducts = async () => {
    try {
      ...
      if (validProducts.length < response.data.length) {
        ...
        toast.warning("Some products could not be displayed due to missing data.");
      }
  ...
  useEffect(() => {
    fetchProducts();
    fetchCategories();
  }, [page, search, selectedCategory, sortStock]);
  ...
  const onSubmit = async (data) => {
    try {
      ...
      let response;
      if (editingProduct) {
        response = await vendorService.updateProduct(editingProduct._id, formData);
        if (response.status === 200) {
        ...
        <div className="flex flex-wrap gap-4 mb-6">
          ...
          <select
            ...
            onChange={(e) => {
              ...
              setSortStock(val === "asc" || val === "desc" ? val : "");
            }}
            className="rounded-md border border-input px-3 py-2"
          >
                  ...
                  {product.images && product.images.length > 0 && (
                    <div className="mb-4">
                      <img
                        ...
                        onError={(e) => {
                          if (e.target.src !== `${BASE_URL}/public/fallback-image.jpg`) {
                  ...
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <div className="flex gap-2">
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => router.push(`/vendor/dashboard/products/${product._id}`)}
                      >
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50 p-4">
          <Card className="w-full max-w-lg">
            <div className="max-h-[90vh] overflow-y-auto custom-scrollbar p-4">
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                ...
                <div>
                  ...
                  <textarea
                    {...register("content")}
                    className={`w-full rounded-md border ${errors.content ? "border-red-500" : "border-input"
                      } px-3 py-2 resize-y`}
                    rows={4}
                ...
                <div>
                  ...
                  <select
                    {...register("categoryId")}
                    className={`w-full rounded-md border ${errors.categoryId ? "border-red-500" : "border-input"
                      } px-3 py-2`}
                  >
                    {categories.length === 0 ? (
